...
DYNAMIC CRITICAL PENALTY: penalty = (1 - sat_CRITICAL) instead of fixed -0.3
"""
...
# DYNAMIC PENALTY: now penalty = (1 - sat_CRITICAL) instead of fixed 0.3
...
class PpoXApp(xAppBase):
    ...
    def _state_reward(self, slc):
        ...
        # Base reward: weighted sum of slice satisfactions
        reward = float(np.dot(W, sat_clip))
        
        # DYNAMIC PENALTY: if CRITICAL unsatisfied, subtract (1 - sat_CRITICAL)
        if sat_clip[0] < 1.0:
            critical_deficit = 1.0 - sat_clip[0]  # e.g., if 90% satisfied, deficit = 0.1
            reward -= critical_deficit
            # Optional: print debug info (commented out for production)
            # print(f"[PENALTY] CRITICAL sat={sat_clip[0]:.3f}, deficit={critical_deficit:.3f}, reward={reward:.3f}")
    ...
    def _loop(self):
        ...
        print(f"[PPO-CTRL] DYNAMIC PENALTY: penalty = (1 - sat_CRITICAL)")
        while self.running:
            ...
            crit_deficit = (1 - sat[0]) if sat[0] < 1.0 else 0.0
            print(f"{t} sat=[{sat[0]:.2f},{sat[1]:.2f},{sat[2]:.2f}] "
                  f"cqi=[{state[3]*15:.0f},{state[4]*15:.0f},{state[5]*15:.0f}] "
                  f"act={action}{PROFILES[action]} r={reward:.3f} pen={crit_deficit:.3f}")
