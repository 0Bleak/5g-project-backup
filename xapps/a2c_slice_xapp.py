#!/usr/bin/env python3
"""
A2C online slice-aware DL-PRB allocator xApp -- 1 DU / 3 slices.

Advantage Actor-Critic (A2C): on-policy actor-critic trained ONLINE in closed
loop on the live srsRAN/OSC-RIC testbed via E2SM-RC. Identical MDP to the PPO
and DQN xApps (same state, action set, reward, mask, network); the ONLY
difference from PPO is the policy-update rule:
  * plain policy-gradient loss (-logp * advantage), no clipped surrogate;
  * a single gradient pass per rollout, no multi-epoch inner loop;
  * no stored old log-prob / probability ratio.
...
Run (inside python_xapp_runner): python3 a2c_slice_xapp.py --interval 1 --train
Eval (frozen, for comparison): python3 a2c_slice_xapp.py --eval --ckpt a2c.pt
"""
...
# A2C hyper-parameters (shared with PPO except no CLIP, no EPOCHS)
ROLLOUT, GAMMA, LAM = 32, 0.99, 0.95
LR, ENT_COEF, VF_COEF = 3e-4, 0.03, 0.5
CKPT_DEFAULT = "/tmp/a2c_slice.pt"
TRAIN_LOG    = "/tmp/a2c_train_log.csv"
...
class A2C:
    def __init__(self, train=True, ckpt=CKPT_DEFAULT):
        ...
        if os.path.exists(ckpt):
            ...
            print(f"[A2C] loaded checkpoint {ckpt}")
    ...
    def maybe_update(self, last_val, update_idx):
        ...
        # ---- A2C update: SINGLE pass, plain policy gradient, no clipping ----
        new_logp, v, entropy = self.net.evaluate(obs, mask, act)
        policy_loss = -(new_logp * adv_t).mean()
...
# ------------------------------ the xApp ------------------------------------
class A2cXApp(xAppBase):
    def __init__(self, c, h, r, interval, a2c):
    ...
    def _loop(self):
        mode = "TRAIN" if self.a2c.train_mode else "EVAL"
        print(f"[A2C-CTRL] {mode} | interval={self.interval}s | rollout={ROLLOUT}")
        while self.running:
            ...
            if self.prev is not None and self.a2c.train_mode:
                o, a, lp, v, msk = self.prev
                self.a2c.store(o, a, lp, v, msk, reward)
                out = self.a2c.maybe_update(last_val=0.0, update_idx=self.update_idx)
                if out:
                    self.update_idx += 1
                    mr, pl, vl, ent = out
            ...
            mask = self._mask(sat)
            action, logp, val = self.a2c.select(state, mask)
...
if __name__ == "__main__":
    ...
    a = p.parse_args()
    train = not a.eval
    a2c = A2C(train=train, ckpt=a.ckpt)
    x = A2cXApp(a.config, a.http_server_port, a.rmr_port, a.interval, a2c)
